...
var pageElmt = document.getElementById("thisPage");
	scanActors.elmts =  pageElmt.getElementsByClassName("actor"); // all potentially scannable, but not necessarily visible buttons
var daBtnsElmt = document.getElementById("daBtns");
	q4Actors.elmts = daBtnsElmt.getElementsByClassName("actor"); // all buttons in the button grid
var ytActorsElmt = document.getElementById("yt_strip");
	ytActors.elmts = ytActorsElmt.getElementsByClassName("actor"); // all buttons in the youtube strip
	scanActors.currIx = 0;
var scanActorsTotal = scanActors.elmts.length;
var q4ActorsTotal = q4Actors.elmts.length;
var ytActorsTotal = ytActors.elmts.length;
//alert("all actors = " + scanActorsTotal + ", q4 actors = " + q4ActorsTotal + ", yt actors = " + ytActorsTotal);
var scanActorsMin = 0;
var scanActorsMax = q4ActorsTotal;
var arrowScan = true;	
...
var q4HidCount = 0;
var ytHiddenActors = [];
var doneSpinning = false;
var lastElement = {};
var skipSpin = false;

var b_spinIt = document.getElementById('spinIt');
b_spinIt.addEventListener('click', toSpinScan, false);

function toSpinScan(event) { // Normally, any click outside an 'actor' button halts tts production. Here we prevent the tts from being stopped.
...
window.addEventListener("load", function(event) {
	...
	var showEditor = localStorage.getItem("showEditBtn");
	if ((showEditor ===null) || (showEditor === "true")) {
	...
	if ((localStorage.getItem("useSpin")=== null)||(localStorage.getItem("useSpin")=== "true")) {
		cb_spinYes.checked = true;
		showElement(wrap_cb_spinDownYes);
		showElement(b_spinIt);
			
	} else if (localStorage.getItem("useSpin")=== "false"){
	...
	if ((localStorage.getItem("useSpinDown")=== null)||(localStorage.getItem("useSpinDown")=== "false")) {
		cb_spinDownYes.checked = false;
		b_spinIt.innerText = "Spin";
	} else if (localStorage.getItem("useSpinDown")=== "true"){
...
document.addEventListener('keydown', function(event) {
	...
	if (event.key == 'ArrowRight') {
		if (arrowScan) {nextActorFocus()};
	}
	if ((event.keyCode == 37) &&(arrowScan)) {
		spinScan();
	}
...
function nextActorFocus(){
				
				if (scanActors.currIx >= scanActorsMax) scanActors.currIx = scanActorsMin;
				let ix = scanActors.currIx;
				if (window.getComputedStyle(scanActors.elmts[ix]).display === 'none'){
					scanActors.currIx++;
				    nextActorFocus();
				} else {
					scanActors.elmts[ix].focus();
					scanActors.currIx++;
				}
			}
...
function checkActions(disThing){
	...
	if (disThing.id == "buttonPause") ytPlayer.pauseVideo();
	if (disThing.id == "buttonPlay") ytPlayerPlay();
	if (disThing.id == "buttonStop") ytStop();
...
function checkCommand(cmdStr, disThing) {
	switch (cmdStr.trim()) {
		...
		case "_pauseAudioAfter120m": setTimeout(function(){audioPlayer.pause();}, 120*60*1000);
			break;
		case "_loadYTVideo": skipSpin = true; getYTVideo(disThing.getAttribute('data-src'));
			break;
		case "_pauseMediaAfter15s": mediaTimer = 15; setTimeout(function(){ pauseMedia();}, mediaTimer*1000);
			break;
		case "_pauseMediaAfter30s": mediaTimer = 30; setTimeout(function(){ pauseMedia();}, mediaTimer*1000);
			break;
		case "_pauseMediaAfter60s": mediaTimer = 60; setTimeout(function(){ pauseMedia();}, mediaTimer*1000);
			break;
...
// check to see if the focus has moved to an actor via Tab key or right arrow key.
...
function spinScan() {
	...
	if (skipSpin) return;
	if (notSpinning) {
	if (doneSpinning && cb_spinDownYes.checked) {
		q4HidCount = q4HiddenActors.push(lastElement);
		if (q4HidCount < q4ActorsTotal) { // check to see if less than all grid buttons have been chosen once
		hideElement(lastElement);
		doneSpinning = false;
		} else { 
			while (q4HiddenActors.length > 0) { // if all buttons were chosen once then show them all again.
				showElement(q4HiddenActors.pop());
			}
		}
	}
	notSpinning = false;	
	nextActorFocus();
	timerID = setInterval("nextActorFocus();",50);
	spinTime = 3000 + (4400 * Math.random());
	tID2 = setTimeout("spinStop();",spinTime);
	tts.DvRate = 1.2;
	tts.ReadText("spinning spinner");
	tts.Pitch = 1.28;
	tts.ReadText("watch it go");
	tts.Pitch = 1.2;
	tts.ReadText("where it stops");
	tts.Pitch = 1.1;
	tts.ReadText("soon we'll know");
	tts.Pitch = 1.0;
	tts.DvRate = 0.75;
	}
   }
...
function spinStop() {
	...
	checkActions(document.activeElement);
