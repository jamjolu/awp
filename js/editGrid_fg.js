...
function showElement(dis){
 ...
 function editOptions () {
	showElement(document.getElementById('options'));
			...
			function back7Actors() {
				setDefaultLook();
				for (let it = 7; 0 < it; it--) {
					editActors.currIx--;
			...
			function setHilitedLook(){
				editActors.elmts[editActors.currIx].style.border = "10px dashed orange";
			...
			function getActorData(thisThing){
				...
				if (tagType === "A") {
					...
					document.getElementById("btnXLink").value  = thisThing.getAttribute('data-src');
				}
